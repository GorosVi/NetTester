Часть системы, реализованная на ресурсах FPGA, обрабатывает задачи генерации тестового трафика, передачу его на подключенную к FPGA ИМС Ethernet PHY, приём сетевого трафика, отбор отправленных системой пакетов и измерение задержки между моментом отправки пакета системой и его приёмом.

Реализация этой части системы может быть разделена на несколько основных блоков:
* Генератор пакетов
* Приёмник пакетов
* Таймер-счётчик времени
* MAC-ядро
* Мост JTAG -> Avalon-MM для управления системой и чтения результатов

Данная архитектура реализована средствами QSYS, в которой включены и настроены как модули собственной разработки, так и IP-ядра от Alter-ы.
Конфигурация модулей осуществляется по шине Avalon-MM-liteweight, используются 32-битные регистры с адресацией по словам. (Big endian, but check!)

MAC-ядро представлено на схеме компонентами MAC_RX и MAC_TX (и представляют независимые интерфейсы одного и того-же модуля)

Для доступа к регистрам конфигурации и результату измерений использован мост JTAG-to-Avalon-MM

Модули генератора пакетов и их приёмника представлены, соответственно, компонентами Transmitter и Receiver.

![System block diagram](Diagram/All%20system.y-ed.png)

Далее - более подробное описание модулей, входящих в состав системы.


Генератор (Timestamp_counter)
=============================
Модуль представляет из себя счётчик, который увеличивается на 1 с тактовой частотой проекта (125 MHz).
Результат выдаётся на 32-битный параллельный интерфейс Avalon conduit. До переполнения счётчика в таких условиях пройдёт около 34 секунд.

Используется для отсчёта времени, прошедшего между отправкой и приёмом каждого из пакетов.

Передатчик
==========

Передатчик выполнен одним модулем. Имеет выходной интерфейс 32-bit Avalon-ST, для подключения на вход MAC.
Также модуль имеет контрольно-статусные регистры, подключенные на интерфейс 32-bit Avalon-MM и вход для данных с генератора timestamp (32-bit Avalon conduit). Флаг Enable разрешает модулю передавать пакеты.
...
Регистры статуса и конфигурации:
* 0x0  Флаги разрешения передачи, остановки передачи, сброса
* 0x1  Длина пакета
* 0x2  Пауза между пакетам в тактах
* 0x3  Число отправляемых пакетов (уменьшается на 1 при отправке каждого пакета)
...
Если смотреть на модуль целиком, сначала он вычитывает данные из MAC-RX сразу, как они там появятся.

Из полученных данных для каждого пакета вычисляется значение измеряемой модулем задержки и сохраняется в FIFO. Здесь стоит иметь ввиду, что для всех пакетов, кроме тех, что отправлены нашей системой, результат будет неправильным. Задача остальных модулей - выбрать из потока данных пакеты, отправленные нашей системой и, соответственно, пригодные для вычисления задержки.

Далее поток данных сетевого трафика передаётся модулям отбора данных - фильтрам и обработчику результата фильтрации. Фильтры помечают наши пакеты флагом tag_own, а обработчик флагов передаёт результат измерения задержки таких пакетов на обработку программной частью системы (в DMA-модуль).
...
Оба флага представлены сигналами в составе интерфейса Avalon-ST.
...
* 0x0  Port-SRC - порт протокола UDP
* 0x1  IP-SRC - IP адрес хоста


Подмодуль Tag_processor
--------------------------

Принимает пакет, флаги от предыдущего модуля и результат измерения задержки из FIFO.

Имеет флаг Enable, по которому начинает передавать результаты изменений. Иначе, просто читает все пакеты и дропает их.
...
Регистры статуса и конфигурации:
* 0x0  Флаги включения модуля и сброса счётчиков статистики
