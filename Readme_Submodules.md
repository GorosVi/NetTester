Модуль, реализующий функционал систем на FPGA, собран из нескольких IP-ядер, часть из которых представлена IP-ядрами Alter-ы.
Основные модули выделены на высокоуровневой схеме (см. ниже) и выполнены в виде IP-ядер (?).

Так как IP-ядра от Altera позволяют из использовать в системе qsys, остальную систему можно собрать в нём. Остальные ядра также разрабатываются с поддержкой qsys.
