...
Используется для отсчёта времени, прошедшего между отправкой и приёмом каждого из пакетов. Его значение увеличивается на 1 с частотой 10кГц, что для 32-битного результата даёт максимальный временной интервал порядка 30 сек с точностью измерения 0.1 мсек.
